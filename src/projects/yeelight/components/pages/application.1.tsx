...
import { ILightModel, IRGBColor, IBaseWeatherSensor, IGatewayModel } from "../../../../interfaces/xiaomi";
    ...
    return (
      <div className="ms-Grid-row">
        <div className="ms-Grid-col ms-sm12">
          ...
          {(!this.state.gateways || this.state.gateways.length < 1) && <h1 className="ms-font-xl ms-fontColor-themePrimary">Keine Gateways gefunden</h1>}
          <div className="ms-Grid-row">
                  ...
                  <div className="ms-Grid-col ms-sm12 ms-lg6 ms-xl3" key={"gwr_" + index}>
                    <BaseLight
                      ...
                      onBrightnessChanged={(lightInformation: ILightModel, brightness: number) => {
                        Axios.post("/api/gateways/" + lightInformation.id + "/brightness/" + brightness).then(this.reloadGatewayInformations);
                      }}
                      onColorChanged={(lightInformation: ILightModel, color: IRGBColor) => {
                        Axios.post("/api/gateways/" + lightInformation.id + "/color", { color }).then(this.reloadGatewayInformations);
                      }}
                      ...
                      onPowerChanged={(lightInformation: ILightModel) => {
                        Axios.post("/api/gateways/" + lightInformation.id + "/power").then(this.reloadGatewayInformations);
                      }}
                    />
                  </div>
                );
