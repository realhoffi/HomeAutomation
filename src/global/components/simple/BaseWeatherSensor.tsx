...
export class BaseWeatherSensor extends React.Component<IBaseWeatherSensorProps, IBaseWeatherSensorState> {
  constructor(props) {
    super(props);
            ...
            {!this.state.showDetails && (
              <Fragment>
                <div className="ms-Grid-row">
                  <div className="ms-Grid-col ms-sm12">
                    <h1 className="ms-font-su ms-fontColor-themePrimary">{this.props.sensorInformations.temperature + " °C"}</h1>
                  </div>
                </div>
                <div className="ms-Grid-row">
                  <div className="ms-Grid-col ms-sm6">
                    <h1 className="ms-font-xl ms-fontColor-themePrimary">
                      ...
                      <i className="ms-Icon ms-Icon--Precipitation" aria-hidden="true" />
                    </h1>
                  </div>
                  {this.props.sensorInformations.hasPressure && (
                    <div className="ms-Grid-col ms-sm6">
                      <h1 className="ms-font-xl ms-fontColor-themePrimary">{this.props.sensorInformations.pressure}</h1>
                    </div>
                  )}
                </div>
              </Fragment>
            )}
            {this.state.showDetails && <BaseWeatherSensorChart sensorInformations={this.props.sensorInformations} />}
          </Panel>
        </div>
      </div>
    );
